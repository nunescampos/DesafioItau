package com.desafioItau.Api.geradorByAluysio;
...
public class GerarClasseJavaApp extends JFrame implements ActionListener,
	...
	private String pathRepoInter = ClienteRepository.class.getResource("")
			.toString().replaceAll(pathJavaReplace, pathJava)
			.replaceAll(fileReplace, "");


	@CheckBoxGeraCodigoAnnotation(descricao = "Entidade")
	private String pathEntidade = Cliente.class.getResource("").toString()
	...
	private void gerarCodigo() throws Exception {
		...
		for (String key : resultado.keySet()) {
			if (resultado.get(key)) {
				if (key.equals("pathRepoInter") && resultado.get(key)) {
					gerarRepoInter();
	...
	private void gerarDeclaracaoBean() {
		...
		builder.append("<bean id=\""+textFieldNomeClasse.getText().toLowerCase()+"Controller\" name=\""+textFieldNomeClasse.getText().toLowerCase()+"Controller\"  class=\"com.desafioItau.Api.Controllers."+textFieldNomeClasse.getText()+"Controller\" />\n");
		builder.append("<bean id=\"repository"+textFieldNomeClasse.getText()+"\" name=\"repository"+textFieldNomeClasse.getText()+"\"  class=\"com.desafioItau.Api.Repository"+textFieldNomeClasse.getText()+"\" />\n");
